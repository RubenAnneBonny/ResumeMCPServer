...
| `get_skim_review_prompt(name, …)` / `get_red_flag_prompt(name, …)` / `get_proofread_prompt(name)` | The same three passes, separately (kept for compatibility). Launch in parallel, never sequentially. |
...
4. **Revise, review, finalize.** Apply blocking fixes and re-`generate_resume` until the verdict is `READY` (cap ~3 rounds; nitpicks are applied silently in the final render, without another critique round). Run the combined final pass (`get_final_review_prompt`), then `finalize_resume(...)` — which refuses unless a critique is registered, the PDF sits in the configured page window (`max_pages`, and `target_pages` when set), and every catalogue job is present if `include_all_experience` is on.

The reviews are the slow part of a run, so the prompts are built to be cheap: the catalogue is embedded compactly with the agent-only `tags` metadata stripped (evidence like `narrative` stays), and every sub-agent prompt **starts with the identical catalogue block** so the calls share a cacheable prefix. In Claude Code, `.claude/agents/` pins each review to a faster model. Claude Code users can also ask for **quick mode** (see `CLAUDE.md`): one critique round, no separate web research when the JD is pasted in full. The gates never get skipped in either mode.
